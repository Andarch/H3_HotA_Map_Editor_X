...
def draw_header() -> None:
    ...
    xprint(type=MsgType.HEADER, text=subheader_default)
    xprint(type=MsgType.HEADER, text=subheader_mapname)
    xprint(type=MsgType.HEADER, text=subheader_mapfile)
    xprint(type=MsgType.HEADER, text=subheader_default)
    xprint(type=MsgType.HEADER, text="")
...
def xprint(
    ...
    def menu_prompt(name: str, items: list) -> str:
        ...
        def print_menu(name: str, items: list, width: int) -> list[str]:
            valid_keys = [] if items == Menu.MAIN["menus"][0] else [KB.ESC.value]
            xprint(text=f"{name}\n", align=Align.CENTER)
            for item in items:
                if item:
