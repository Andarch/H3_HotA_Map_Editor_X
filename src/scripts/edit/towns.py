...
def modify(spells: bool = False, buildings: bool = False, events: bool = False) -> None:
    if spells:
        _enable_spells()
    if buildings:
        _enable_buildings()
...
def _enable_spells() -> None:
    xprint(type=Text.ACTION, text="Enabling spell research and all spells in all towns...")
...
def _enable_buildings() -> None:
    xprint(type=Text.ACTION, text="Enabling all buildings in all towns...")
    for obj in map_data["object_data"]:
        if obj["id"] in TOWN_IDS:
...
def _create_events() -> None:
    xprint(type=Text.ACTION, text="Configuring town events...")
    for obj in map_data["object_data"]:
